...
const platforms = [
  {
    ...
    icon: '🪟',
    versions: 'Windows 10+',
    downloadUrl: 'https://github.com/yeflyleaf/Guli-MusicAttendant/releases/latest/download/GL_Music.Setup.exe',
    fileName: 'GL_Music.Setup.exe',
    recommended: true,
  },
  {
    name: 'macOS',
...
<template>
  <section id="download" class="section download-section">
    ...
    <div class="container relative-z">
      <h2 class="section-title animate-on-scroll"><span class="gradient-text">下载</span>体验</h2>
      <p class="section-subtitle animate-on-scroll delay-1">
        提供各平台打包好的构建版本，也支持源码编译安装，完全开源免费
      </p>
      ...
      <div class="download-grid">
        <div v-for="(platform, index) in platforms" :key="platform.name"
          ...
          <a :href="platform.downloadUrl" class="btn btn-primary platform-download-btn">
            立即下载
          </a>
          <div class="file-name">{{ platform.fileName }}</div>
        </div>
      </div>
      ...
      <div class="getting-started animate-on-scroll delay-4">
        ...
        <div class="code-block glass-card code-block-dynamic">
          <div class="code-header">
            ...
            <button class="code-copy" id="copy-install-code" title="复制" @click="
              copyCode(
                'git clone https://github.com/yeflyleaf/Guli-MusicAttendant.git\ncd Guli-MusicAttendant/Frontend\nnpm install\nnpm run dev\n# 构建生产版本\nnpm run build:win\nnpm run build:mac\nnpm run build:linux',
              )
              ">
              📋
            </button>
...
<span class="code-cmd">git clone</span> https://github.com/yeflyleaf/Guli-MusicAttendant.git
<span class="code-cmd">cd</span> Guli-MusicAttendant/Frontend
...
<span class="code-comment"># 构建各平台生产版本</span>
<span class="code-cmd">npm run</span> build:win   <span class="code-comment"># Windows</span>
<span class="code-cmd">npm run</span> build:mac   <span class="code-comment"># macOS</span>
<span class="code-cmd">npm run</span> build:linux <span class="code-comment"># Linux</span></code></pre>
        </div>
      </div>
    </div>
  </section>

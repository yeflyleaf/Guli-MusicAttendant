...
const cardRotations = ref(features.map(() => ({ rx: 0, ry: 0 })))
...
const handleCardMove = (e: MouseEvent, index: number) => {
  ...
  cardRotations.value[index] = { rx: rotateX, ry: rotateY }
}
...
const resetCardMove = (index: number) => {
  ...
  cardRotations.value[index] = { rx: 0, ry: 0 }
}
</script>
  ...
  <section id="features" class="section features-section">
    ...
    <div class="container">
      ...
      <div class="features-grid">
        <div v-for="(feature, index) in features" :key="feature.title" class="feature-card glass-card animate-on-scroll"
          :class="['delay-' + ((index % 3) + 1), { 'is-active': activeIndex === index }]" :style="{
            '--card-gradient': feature.gradient,
            '--card-border': feature.borderColor,
            transform: `perspective(1000px) scale(${activeIndex === index ? 1.05 : 1}) rotateX(${cardRotations[index]?.rx || 0}deg) rotateY(${cardRotations[index]?.ry || 0}deg) translateZ(${activeIndex === index ? '20px' : '0'})`,
          }" :id="'feature-card-' + index" @mouseenter="activeIndex = index" @mousemove="handleCardMove($event, index)"
          @mouseleave="resetCardMove(index)" @click="activeIndex = index">
          <div class="feature-icon-wrapper" :style="{
            transform: `translateZ(40px) ${activeIndex === index ? 'scale(1.1) rotate(5deg)' : ''}`,
          }">
            <span class="feature-icon">{{ feature.icon }}</span>
          </div>
          <h3 class="feature-title" :style="{ transform: 'translateZ(30px)' }">
            {{ feature.title }}
          </h3>
          <p class="feature-description" :style="{ transform: 'translateZ(20px)' }">
            {{ feature.description }}
          </p>

          <div v-if="activeIndex === index" class="card-glare" :style="{

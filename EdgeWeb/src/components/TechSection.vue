...
const perfItems = [
    ...
    { label: '渐进式渲染', detail: '首屏 20 条 + requestAnimationFrame 补全', icon: '📊', progress: 90 },
...
<template>
    <section id="tech" class="section tech-section">
        ...
        <div class="container relative-z">
            ...
            <!-- 技术栈网格 -->
            <div class="tech-grid">
                <a v-for="(tech, index) in techStack" :key="tech.name" :href="tech.url" target="_blank"
                    rel="noopener noreferrer" class="tech-card animate-on-scroll" :class="'delay-' + ((index % 4) + 1)"
                    :id="'tech-card-' + tech.name.toLowerCase().replace(/\s/g, '-')" @mouseenter="hoveredTech = index"
                    @mouseleave="hoveredTech = null" :style="{
                        '--tech-color': tech.color,
                        borderColor: hoveredTech === index ? tech.color + '60' : '',
                        boxShadow: hoveredTech === index ? `0 0 30px ${tech.color}20, 0 10px 30px rgba(0,0,0,0.3)` : '',
                    }">
                    <div class="tech-icon"
                    ...
                    <div class="tech-info">
                        <div class="tech-name">
                            {{ tech.name }}
                            <span v-if="tech.version" class="tech-version" :style="{
                    ...
                    <div class="tech-card-highlight"
                        :style="{ background: `linear-gradient(90deg, transparent, ${tech.color}40, transparent)` }">
                    </div>
                </a>
            </div>
            ...
            <div class="perf-section animate-on-scroll">
                ...
                <div class="perf-grid">
                    <div v-for="(item, index) in perfItems" :key="item.label" class="perf-item animate-on-scroll"
                        :class="'delay-' + ((index % 3) + 1)" :id="'perf-item-' + index"
                        @mouseenter="hoveredPerf = index" @mouseleave="hoveredPerf = null">
                        <span class="perf-icon" :class="{ 'perf-icon-active': hoveredPerf === index }">{{ item.icon
                        }}</span>
                        <div class="perf-content">
                            ...
                            <div class="perf-progress-bar">
                                <div class="perf-progress-fill" :style="{
...
.perf-section::before {
    ...
    right: 0;
    height: 1px;
    background: linear-gradient(90deg, transparent, rgba(0, 229, 255, 0.3), rgba(124, 77, 255, 0.3), transparent);
}
...
.perf-progress-glow {
    ...
    border-radius: 50%;
    box-shadow: 0 0 10px var(--color-accent-cyan), 0 0 20px rgba(0, 229, 255, 0.3);
}

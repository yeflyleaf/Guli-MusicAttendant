...
export function setupMusicIpc(): void {
  ...
  // 移除最近播放记录
  ipcMain.handle('music:removeRecentlyPlayed', (_event, ids: number[]) => {
    return musicRepo.removeRecentlyPlayed(ids)
  })

  // 清空最近播放记录
  ipcMain.handle('music:clearRecentlyPlayed', () => {
    return musicRepo.clearRecentlyPlayed()
  })

...
> **专业级本地音乐管理系统** — 基于 Electron + Vue 3 + TypeScript + Vite + Element Plus + SQLite 构建的现代化桌面音乐播放器

[功能特性](#-功能特性) • [快速开始](#-快速开始) • [技术架构](#-技术架构) • [项目结构](#-项目结构) • [使用说明](#-使用说明)
...
在应用启动前并行加载设置和音乐库数据，充分利用初始化阶段的空闲时间。

### 2. 启动屏缓冲

精美的启动动画作为数据加载缓冲期，优化用户感知的启动速度。

### 3. 渐进式渲染

- 首屏仅渲染 20 条数据，确保页面切换瞬间完成
- 利用 `requestAnimationFrame` 异步补全剩余数据
- 彻底消除列表切换卡顿

### 4. 视图层持久化

使用 `<keep-alive>` 缓存页面组件，实现零开销的页面切回。

### 5. GPU 硬件加速

显式启用 GPU 加速，将图形渲染任务卸载至 GPU。

### 6. 帧率节流

音频可视化限制在 30 FPS，在保持流畅度的同时减少 50% CPU 占用。
...
- **开发环境**：数据存储于 `Frontend/data/`
- **生产环境**：数据存储于应用安装目录下的 `data/`
...
Made with ❤️ by [yeflyleaf](https://github.com/yeflyleaf) | ⭐ 如果这个项目对你有帮助，请给一个 Star！
